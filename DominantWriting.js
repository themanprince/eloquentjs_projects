...
//and my dominant direction function... finally written after first writing an algorithm
let arr = [];
let dominantDir = (text) => {
  for(char of text) {
    if(char === " ")
      continue;
    let script = characterScript(char.charCodeAt(0));
    arr.push(script.direction);
  }
  //assuming all the directions have been added into the arr
  //now to get the one with the greatest count
  let countsArr = countBy(arr, (e => e));
  //looping all objects in countsArr to get the dir with the highest count
  let dominantDir = countsArr[0];
  for(let countObj of countsArr) {
    if(countObj.count > dominantDir.count)
      dominantDir = countObj;
  }
